...
output_file <- "R_output2.csv"

######################################################################
# Do not amend the code below
px <- mort_data$SURV_PROB
rpx <- mort_data$NEWSURV_LOWER
...
dist_factor <- (1 + int_data$RATE) ^ (-1)#lower bound
...
for (i in 1:length(mort_load)){
  ...
  if (pol_type[i] == 1){
    ...
    PREMIUM1[i] = pol_prem * sum(pol_rtpx[1:20] * pol_vt[1:20] *  no_lapse_rate)
    PROFIT1[i]=PREMIUM1[i]-EXPENSE1[i]- BENEFIT1[i]
  }
}
# Export csv file 
