...
for (i in 1:length(mort_load)){
  ...
  if (pol_type[i] == 1){
    ...
    PREMIUM1[i] = PREMIUM0[i]* sum(pol_rtpx[1:20] * pol_vt[1:20] *  no_lapse_rate)
    PROFIT1[i]=PREMIUM1[i]-EXPENSE1[i]- BENEFIT1[i]
  }
}
# Export csv file 
pricing_data <- data.frame(BENEFIT0, EXPENSE0, PREMIUM0, BENEFIT1, EXPENSE1, PREMIUM1, PROFIT1)
